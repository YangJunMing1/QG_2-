...
void GameApp::UpdateScene(float dt)
{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D2) && m_CurrMode != ShowMode::FireAnim)
	{
		...
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTestAnims[0].GetAddressOf());
		//m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFireAnims[0].GetAddressOf());
	}
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D3) && m_CurrMode != ShowMode::RoAnim)
	{
		// 播放旋转纹理木箱动画
		m_CurrMode = ShowMode::RoAnim;
		m_pd3dImmediateContext->IASetInputLayout(m_pVertexLayout3D.Get());
		...
		m_pd3dImmediateContext->VSSetShader(m_pVertexShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShader(m_pPixelShader3D.Get(), nullptr, 0);
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pMix.GetAddressOf());
	}
	...
	if (m_CurrMode == ShowMode::WoodCrate || m_CurrMode == ShowMode::RoAnim)
	{
		static float phi = 0.0f, theta = 0.0f;

		Mouse::State Mouse_State = m_pMouse->GetState();	//得到当前一帧鼠标的状态
		Mouse::State Mouse_LastState = m_MouseTracker.GetLastState();

		m_MouseTracker.Update(Mouse_State);	//将鼠标的状态更新到鼠标状态跟踪器
		if (Mouse_State.leftButton = true && m_MouseTracker.leftButton == m_MouseTracker.HELD)
		{

			theta += (Mouse_LastState.x - Mouse_State.x) * 0.01f;
			phi -= (Mouse_State.y - Mouse_LastState.y) * 0.01f;
		}
	...
	else if (m_CurrMode == ShowMode::FireAnim)
	{
		...
		if (totDeltaTime > 1.0f / 60)
		{
			...
			m_CurrFrame = (m_CurrFrame + 1) % 442;
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pTestAnims[m_CurrFrame].GetAddressOf());
		}
	}
}
...
void GameApp::DrawScene()
{
	...
	for (int i = 0; i < m_IndexCount; i += 3)
	{
		...
		else if (m_CurrMode == ShowMode::RoAnim)
		{
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pMix.GetAddressOf());
			m_pd3dImmediateContext->DrawIndexed(m_IndexCount, 0, 0);
		}
	...
	if (m_pd2dRenderTarget != nullptr)
	{
		...
		static const WCHAR* textStr = L"切换显示: 1-木箱(3D) 2-动画(2D) 3-旋转纹理\n4-开关灯\n";
		m_pd2dRenderTarget->DrawTextW(textStr, (UINT)wcslen(textStr), m_pTextFormat.Get(),
			D2D1_RECT_F{ 0.0f, 0.0f, 600.0f, 200.0f }, m_pColorBrush.Get());
		static const WCHAR* textStr2 = L"2021QG工作室图形组第4次小组培训\n杨骏铭 2021年4月23日\n";
		m_pd2dRenderTarget->DrawTextW(textStr2, (UINT)wcslen(textStr2), m_pTextFormat.Get(),
			D2D1_RECT_F{ 450.0f, 0.0f, 1000.0f, 700.0f }, m_pColorBrush.Get());
...
bool GameApp::InitResource()
{
	...
	// 初始化木箱纹理
	/*HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\WoodCreat.dds", nullptr, m_pWoodCrate.GetAddressOf()));*/
	...
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\flareoutputA.dds", nullptr, m_pMix.GetAddressOf()));
	...
	WCHAR strTest[40];
	m_pTestAnims.resize(442);

	for (int i = 1; i <= 442; ++i)
	{
		wsprintf(strTest, L"..\\Texture\\Test\\Test  %03d.bmp", i);
		HR(CreateWICTextureFromFile(m_pd3dDevice.Get(), strTest, nullptr, m_pTestAnims[static_cast<size_t>(i) - 1].GetAddressOf()));
	}
	//WCHAR strFile[40];
